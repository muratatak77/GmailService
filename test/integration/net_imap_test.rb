...
class UidValidityTester
  ...
  def select_without_uid_validity_checks(mailbox); end
end
...
class NetImapTest < ActiveSupport::TestCase
  ...
  test 'serialize address' do
    ...
    assert_equal 'foo@bar.com',
                 Net::IMAP::Address.new('Foo Bar', nil, 'foo', 'bar.com').serialize(include_name: false)
  ...
  test 'parse_namespace_response' do
    ...
    assert_equal({ personal: [Net::IMAP::Namespace.new('', '/')],
                   ...
                   shared: [Net::IMAP::Namespace.new('#shared/', '/'), Net::IMAP::Namespace.new('#public/', '/'),
                            Net::IMAP::Namespace.new('#ftp/', '/'), Net::IMAP::Namespace.new('#news.', '.')] },
  ...
  test 'retry_select_fails' do
    ...
    assert_equal "Could not select mailbox after #{ImapMixin::SELECT_RETRY_ATTEMPTS} tries last response was:Mailbox in use. Please try again later",
  ...
  test 'ensure_nz_number accepts invalid large uids SB-7204' do
    set = Net::IMAP::MessageSet.new(18_446_744_073_709_551_615)
    assert_nothing_raised { set.validate }
  end

  test 'serialize ascii-8bit mailboxes' do
    address = Net::IMAP::Address.from_hash(mailbox: "Caf\xC3\xA9CupSale".force_encoding(Encoding::ASCII_8BIT),
                                           host: 'cutscheaped.eu', name: "Caf\xC3\xA9 Cup Sale".force_encoding(Encoding::ASCII_8BIT))
  ...
  test 'initialize with a hardcoded ssl_version' do
    ...
    Net::IMAP.any_instance.expects(:start_tls_session).with({ ssl_version: :TEST_TLS,
                                                              verify_mode: 1 }).raises(OpenSSL::SSL::SSLError.new).once
  ...
  test 'initialize calls start_tls_session with different TLS types' do
    ...
    Net::IMAP.any_instance.expects(:start_tls_session).with({ ssl_version: :TLSv1,
                                                              verify_mode: 0 }).raises(OpenSSL::SSL::SSLError.new).once
    Net::IMAP.any_instance.expects(:start_tls_session).with({ ssl_version: :SSLv3_client,
                                                              verify_mode: 0 }).raises(OpenSSL::SSL::SSLError.new).once
