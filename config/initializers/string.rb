class String
  ...
  def convert_unknown_to_utf8
    ...
    detected = Encoding::ISO_8859_1.name if detected =~ /iso-8859-2/i

    # If the string is utf-8, try to salvage valid utf-8 from it
    if detected.casecmp(Encoding::UTF_8.name) == 0
      begin
        return encode(Encoding::UTF_16, Encoding::UTF_8, invalid: :replace, undef: :replace).encode(
    ...
    begin
      encode(Encoding::UTF_8, Mail::RubyVer.pick_encoding(detected), invalid: :replace, undef: :replace)
    rescue Encoding::CompatibilityError, Encoding::ConverterNotFoundError, Encoding::UndefinedConversionError,
           Encoding::InvalidByteSequenceError, Errno::EINVAL
      nil
    end
  end
  ...
  def force_valid_utf8!
    ...
    return force_encoding(Encoding::UTF_8) if encoding == Encoding::ASCII_8BIT && is_utf8?
  ...
  def separate_first_and_last_name
    ...
    if combined_name =~ /^([^\[]*)\[/
      front = ::Regexp.last_match(1).gsub(/\s+$/, '')
      back = (::Regexp.last_match(1).gsub(/^\s+/, '') if combined_name =~ /\]([^\]]*)$/)
    ...
    if combined_name =~ /^\s*([^,]+?)\s*,\s*([^,]+?)(\s+\S+)?\s*$/
      ...
      middle = ::Regexp.last_match(3)
      last = ::Regexp.last_match(1)
      unless middle.nil?
        middle = strip_middle_name(middle)
        first += ' ' + middle unless middle.nil? || middle.empty?
      end
  ...
  def separate_names
    return unless (parts = separate_first_and_last_name.try(:compact))

    parts.map { |i| i.split(/\s+/) }.flatten.map do |i|
      i.strip if i.present?
    end
  end
  ...
  def from_q_encoding
    begin
      ...
      Mail::Encodings.unquote_and_convert_to(convert_unknown_to_utf8, Encoding::UTF_8)
    rescue Encoding::CompatibilityError, Encoding::ConverterNotFoundError, Encoding::UndefinedConversionError,
           Encoding::InvalidByteSequenceError, Errno::EINVAL
      self
  ...
  def downcase_with_encoding_workaround
    downcase_without_encoding_workaround
  rescue ArgumentError
    dup.tap do |working_copy|
  ...
  def to_bool
    ...
    return false if self == false || blank? || self =~ /\A(false|f|no|n|0)\z/i
  ...
  def truncate_utf8(byte_size)
    truncated = convert_unknown_to_utf8
    keep_length = truncated.length
    while truncated.bytesize > byte_size
      keep_length -= 1
      truncated = dup if truncated.equal?(self)
